...
@section('content')
    <div class="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-1 text-sm text-slate-600">update a news item</p>
        ...
        <form class="mt-6 space-y-4" method="POST" action="{{ route('admin.news.update', $news) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-slate-900"
                       ...
                       value="{{ old('published_at', optional($news->published_at)->format('Y-m-d\TH:i')) }}" required>
            </div>

            <div>
                <label class="text-sm font-medium">content</label>
                <textarea class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 outline-none focus:border-slate-900"
                          name="content" rows="6" required>{{ old('content', $news->content) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input class="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2"
                       type="file" name="image" accept="image/*">
                <p class="mt-1 text-xs text-slate-500">uploading a new image will replace the old one</p>
            </div>
            ...
            <div>
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    @php
                        $selected = old('tags', $news->tags->pluck('id')->toArray());
                    @endphp
                    ...
                    @foreach ($tags as $tag)
                        <label class="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1 text-sm">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}"
                                ...
                                {{ in_array($tag->id, $selected) ? 'checked' : '' }}>
                            <span>{{ $tag->name }}</span>
                        </label>
                    @endforeach
                </div>
